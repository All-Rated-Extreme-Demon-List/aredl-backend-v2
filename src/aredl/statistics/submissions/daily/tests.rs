...
async fn submission_stats_filter_moderator() {
    ...
    let uri =
        format!("/aredl/statistics/submissions/daily?reviewer_id={mod_id}&page=1&per_page=10");
...
async fn submission_stats_reviewer_filter_returns_empty_for_hidden_reviewer_without_audit() {
    ...
    let uri = format!(
        "/aredl/statistics/submissions/daily?reviewer_id={hidden_reviewer}&page=1&per_page=10"
    );
...
async fn submission_leaderboard_ignores_include_hidden_reviewers_without_audit() {
    let (app, db, auth, _db) = init_test_app().await;
    let (hidden_reviewer, _) = create_test_hidden_reviewer(&db).await;
    let (full_reviewer, _) = create_test_full_reviewer(&db).await;
    let (requester_non_auditor, _) = create_test_full_reviewer(&db).await;
    let (requester_auditor, _) = create_test_auditor(&db).await;
    ...
    let uri = "/aredl/statistics/submissions/daily/leaderboard?include_hidden_reviewers=true";
...
async fn submission_leaderboard_since_filters_out_future_date() {
    ...
    let uri = format!("/aredl/statistics/submissions/daily/leaderboard?since={tomorrow}");
