...
async fn get_mutual_victors() {
    ...
    let req = test::TestRequest::get()
        .uri(&format!(
            "/aredl/records/mutual-victors?level_id={first_level}&other_level_id={second_level}"
        ))
        .insert_header(("Authorization", format!("Bearer {token}")))
        .to_request();
    let resp = test::call_service(&app, req).await;
    ...
    let req = test::TestRequest::get()
        .uri(&format!(
            "/aredl/records/mutual-victors?level_id={first_level}&other_level_id={second_level}&high_extremes=true"
        ))
        .insert_header(("Authorization", format!("Bearer {token}")))
        .to_request();
    let resp = test::call_service(&app, req).await;
...
async fn update_timestamp_endpoint_fetches_youtube_published_at() {
    ...
    let update_req = test::TestRequest::patch()
        .uri(&format!("/aredl/records/{record_id}/update-timestamp"))
        .insert_header(("Authorization", format!("Bearer {token}")))
        .to_request();
