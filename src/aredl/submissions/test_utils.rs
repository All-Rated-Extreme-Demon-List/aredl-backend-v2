...
pub fn set_test_submission_reviewer(
    ...
    diesel::update(submissions::table.filter(submissions::id.eq(submission_id)))
        ...
        .execute(&mut db.connection().unwrap())
        .expect("Failed to set test aredl submission reviewer");
}
...
pub fn set_test_submission_status(
    ...
    diesel::update(submissions::table.filter(submissions::id.eq(submission_id)))
        ...
        .execute(&mut db.connection().unwrap())
        .expect("Failed to set test aredl submission status");
}
...
pub fn test_submission_priorities(
    ...
    submissions::table
        ...
        .expect("Failed to load test aredl submission priorities")
        .into_iter()
        .collect()
}
...
pub fn set_test_submission_raw_url_status_and_reviewer(
    ...
    diesel::update(submissions::table.filter(submissions::id.eq(submission_id)))
        ...
        .execute(&mut db.connection().unwrap())
        .expect("Failed to set test aredl submission review state");
}
...
#[cfg(test)]
pub fn set_test_submission_locked(db: &Arc<DbAppState>, submission_id: Uuid, locked: bool) {
    diesel::update(submissions::table.filter(submissions::id.eq(submission_id)))
        .set(submissions::locked.eq(locked))
        .execute(&mut db.connection().unwrap())
        .expect("Failed to set test aredl submission locked flag");
}
...
pub fn get_test_submission(db: &Arc<DbAppState>, submission_id: Uuid) -> Submission {
    get_test_submission_optional(db, submission_id).expect("Failed to fetch test aredl submission")
}
...
pub fn get_test_submission_optional(
    ...
    submissions::table
        ...
        .optional()
        .expect("Failed to fetch test aredl submission")
}
...
pub fn latest_test_submission_history(
    ...
    submission_history::table
        ...
        .expect("Failed to get test aredl submission history")
}

#[cfg(test)]
pub fn test_submission_history_count(db: &Arc<DbAppState>, submission_id: Uuid) -> i64 {
    submission_history::table
        .filter(submission_history::submission_id.eq(submission_id))
        .count()
        .get_result(&mut db.connection().unwrap())
        .expect("Failed to count test aredl submission history")
}
...
pub fn set_test_submission_history_reviewer(
    ...
    .execute(&mut db.connection().unwrap())
    .expect("Failed to set test aredl submission history reviewer");
}
...
pub async fn create_regular_queue_order_test_submissions(
    ...
    diesel::update(submissions::table.filter(submissions::id.eq(older_created)))
        ...
        .execute(&mut db.connection().unwrap())
        .expect("Failed to set older test aredl submission created_at");

    diesel::update(submissions::table.filter(submissions::id.eq(newer_created)))
        .set(submissions::created_at.eq(late_created))
        .execute(&mut db.connection().unwrap())
        .expect("Failed to set newer test aredl submission created_at");
...
pub async fn create_priority_queue_order_test_submissions(
    ...
    diesel::update(submissions::table.filter(submissions::id.eq(newer_created)))
        ...
        .execute(&mut db.connection().unwrap())
        .expect("Failed to set first priority test aredl submission");
    ...
    diesel::update(submissions::table.filter(submissions::id.eq(older_created)))
        ...
        .execute(&mut db.connection().unwrap())
        .expect("Failed to set second priority test aredl submission");
