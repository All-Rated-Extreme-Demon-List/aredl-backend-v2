...
async fn parse_timezone_from_string() {
    ...
    assert_eq!(
        timezone.name(),
        string,
        "Parsed timezone name does not match"
    );
    ...
    assert!(
        result.is_err(),
        "Parsing invalid timezone should return an error"
    );
}
...
async fn create_shift_instantly() {
    ...
    let staff2 = create_test_user(&db, Some(Permission::SubmissionReview))
        ...
        .0;
    let admin_token =
        create_test_token(admin, &auth.jwt_encoding_key).expect("Failed to generate token");
    let staff_token =
        create_test_token(staff, &auth.jwt_encoding_key).expect("Failed to generate token");
    ...
    assert!(
        ...
        "Failed to create shift, status is {}",
        resp.status()
    );
    let body: serde_json::Value = read_body_json(resp).await;
    ...
    assert_eq!(
        ...
        staff.to_string(),
        "Shift should be assigned to staff"
    );
    assert_eq!(
        body["target_count"].as_i64().unwrap(),
        5,
        "Target count should match"
    );
