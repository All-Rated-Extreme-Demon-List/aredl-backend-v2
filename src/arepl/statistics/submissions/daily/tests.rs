...
async fn submission_stats_filter_level() {
    ...
    let uri = format!("/arepl/statistics/submissions/daily?level_id={level_id}&page=1&per_page=10");
    let req = test::TestRequest::get()
        .uri(&uri)
        .insert_header((header::AUTHORIZATION, format!("Bearer {token}")))
        .to_request();
...
async fn submission_leaderboard_ignores_include_hidden_reviewers_without_audit() {
    ...
    assert_eq!(arr.len(), 1);
    assert_eq!(arr[0].reviewer.id, visible_reviewer);
    ...
    assert!(arr.iter().any(|row| row.reviewer.id == visible_reviewer));
}
