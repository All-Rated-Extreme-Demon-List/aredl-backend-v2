#[cfg(test)]
use crate::arepl::shifts::test_utils::{create_test_recurring_shift, create_test_shift};
#[cfg(test)]
...
async fn patch_shift() {
    ...
    let req = test::TestRequest::patch()
        .uri(&format!("/arepl/shifts/{}", shift_id))
        .insert_header(("Authorization", format!("Bearer {}", token)))
        .set_json(&patch_data)
        .to_request();
    let resp = test::call_service(&app, req).await;
...
async fn patch_recurring_shift() {
    ...
    let req = test::TestRequest::patch()
        .uri(&format!("/arepl/shifts/recurring/{}", recurring_id))
        .insert_header(("Authorization", format!("Bearer {}", token)))
        .set_json(&patch_data)
        .to_request();
    let resp = test::call_service(&app, req).await;
