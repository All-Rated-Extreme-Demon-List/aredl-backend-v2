...
use crate::arepl::records::PublicRecordResolvedWithCountry;
...
#[utoipa::path(
    ...
    responses(
        (status = 200, body = [PublicRecordResolvedWithCountry])
    ),
)]
...
async fn find_all(
    ...
    let records =
        web::block(move || PublicRecordResolvedWithCountry::find_all_by_level(db, level_id)).await??;
