...
async fn find_all(
    ...
    let creators = web::block(move || BaseUser::arepl_find_all_creators(db, level_id)).await??;
...
#[openapi(
    tags(
        (name = "AREDL (P) - Levels (Creators)", description = "Endpoints for fetching and managing the creators list of a specific platformer level"),
    ),
    paths(
        find_all,
        add,
