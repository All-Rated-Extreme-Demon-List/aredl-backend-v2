...
async fn actions_increment_shift() {
    ...
    let req = test::TestRequest::post()
        .uri(&format!("/arepl/submissions/{sub_id}/accept"))
        .insert_header(("Authorization", format!("Bearer {}", token_mod)))
        .set_json(&notes)
        .to_request();
    let resp = test::call_service(&app, req).await;
...
async fn unclaim_submission() {
    ...
    let req = test::TestRequest::post()
        .uri(&format!("/arepl/submissions/{submission_id}/unclaim"))
        .insert_header(("Authorization", format!("Bearer {}", token_mod)))
        .to_request();
    let resp = test::call_service(&app, req).await;
    assert!(resp.status().is_success());
...
async fn accept_existing_record() {
    ...
    let level = create_test_level(&mut conn).await;
    let existing_record =
        crate::arepl::records::test_utils::create_test_record(&mut conn, user_id, level).await;
    ...
    let req = test::TestRequest::post()
        .uri(&format!("/arepl/submissions/{submission}/accept"))
        .insert_header(("Authorization", format!("Bearer {}", token_mod)))
        .set_json(&notes)
        .to_request();
    let resp = test::call_service(&app, req).await;
...
async fn deny_already_denied() {
    ...
    let notes = json!({"notes":"hi"});
    let req = test::TestRequest::post()
        .uri(&format!("/arepl/submissions/{sub_id}/deny"))
        .insert_header(("Authorization", format!("Bearer {}", token)))
        .set_json(&notes)
        .to_request();
    let resp = test::call_service(&app, req).await;
...
async fn under_consideration_already_uc() {
    ...
    let notes = json!({"notes":"ok"});
    let req = test::TestRequest::post()
        .uri(&format!("/arepl/submissions/{sub_id}/underconsideration"))
        .insert_header(("Authorization", format!("Bearer {}", token)))
        .set_json(&notes)
        .to_request();
    let resp = test::call_service(&app, req).await;
