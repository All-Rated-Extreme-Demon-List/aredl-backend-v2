...
async fn submission_leaderboard_counts_and_ordering() {
    ...
    insert_history_entry(sub1, Some(mod1), SubmissionStatus::Accepted, &mut conn).await;
    insert_history_entry(sub1, Some(mod1), SubmissionStatus::Accepted, &mut conn).await;
    insert_history_entry(sub1, Some(mod1), SubmissionStatus::Denied, &mut conn).await;

    let sub2 = create_test_submission(lvl, Uuid::new_v4(), &mut conn).await;
    insert_history_entry(sub2, Some(mod2), SubmissionStatus::Accepted, &mut conn).await;
