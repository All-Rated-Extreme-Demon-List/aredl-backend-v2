...
#[actix_web::test]
async fn get_records_sort_shortest_completion_time() {
    let (app, db, auth, _) = init_test_app().await;
    let (user_id, _) = create_test_user(&db, Some(Permission::RecordModify)).await;
    let token = create_test_token(user_id, &auth.jwt_encoding_key).unwrap();

    let (slower, faster) = create_two_test_records_with_different_timestamps(&db, user_id).await;
...
#[actix_web::test]
async fn get_records_sort_longest_completion_time() {
    let (app, db, auth, _) = init_test_app().await;
    let (user_id, _) = create_test_user(&db, Some(Permission::RecordModify)).await;
    let token = create_test_token(user_id, &auth.jwt_encoding_key).unwrap();
