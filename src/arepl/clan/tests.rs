...
async fn get_clan() {
    ...
    let req = test::TestRequest::get()
        .uri(format!("/arepl/clan/{clan_id}").as_str())
        .to_request();
...
#[actix_web::test]
async fn get_clan_includes_completion_counts_and_member_points() {
    let (app, db, _, _) = init_test_app().await;
    ...
    let req = test::TestRequest::get()
        .uri(format!("/arepl/clan/{clan_id}/levels/{shared_level}/records").as_str())
        .to_request();
