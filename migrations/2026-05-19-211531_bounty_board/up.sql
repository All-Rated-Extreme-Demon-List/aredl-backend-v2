CREATE TYPE bounty_type AS ENUM ('Bounty', 'Weekly', 'Monthly', 'Event');
CREATE TYPE bounty_difficulty AS ENUM ('Easy', 'Medium', 'Hard');

CREATE TABLE aredl.bounties (
	id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
	level_id UUID NOT NULL REFERENCES aredl.levels(id) ON DELETE CASCADE,
...
CREATE TABLE aredl.bounty_completed (
	...
	bounty_id UUID NOT NULL REFERENCES aredl.bounties(id) ON DELETE CASCADE,
...
CREATE TABLE arepl.bounties (
	id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
	level_id UUID NOT NULL REFERENCES arepl.levels(id) ON DELETE CASCADE,
	bounty_type bounty_type NOT NULL,
	bounty_difficulty bounty_difficulty NOT NULL,
	start_date TIMESTAMPTZ NOT NULL,
	end_date TIMESTAMPTZ,
...
CREATE TABLE arepl.bounty_completed (
	user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
	bounty_id UUID NOT NULL REFERENCES arepl.bounties(id) ON DELETE CASCADE,
	completed_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
	PRIMARY KEY (user_id, bounty_id)
);
